    ...
    if (formState.currentState!.validate()) {
      ...
      if (StatusRequest.success == statusRequest) {
        if (response['status'] == "success") {
            ...
            myServices.sharedPreferences.setString("birth", response['data']['date_of_birth'].toString());
            myServices.sharedPreferences.setString("step", "2");
            Get.offNamed(AppRoute.homeScreen);
            Get.snackbar("${myServices.sharedPreferences.getString("username")} ",
                translateDataBase("شكرا لك على انضمامك الى IRIS WHEEL ASSIST", "Thank you for joining IRIS WHEEL ASSIST"),
                icon: IconButton(
                    onPressed: () {},
                    icon: const Icon(
                      IconBroken.Profile,
